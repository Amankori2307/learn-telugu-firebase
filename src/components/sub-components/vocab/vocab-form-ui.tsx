...
import { VocabularyTypeEnum } from "../../../interfaces/vocab.interfaces";
...
const VocabFormUI: React.FC<VocabFormUIProps> = ({
  ...
  return (
    <form
      ...
      className="space-y-6 p-4 w-full max-w-2xl mx-auto"
    >
      {/* Type Field */}
      <InputField
        label="Type"
        ...
        onBlur={handleBlur}
        as="select"
        options={Object.entries(VocabularyTypeEnum).map(([key, value]) => ({
          label: key,
          value: value,
        }))}
        error={errors.type}
